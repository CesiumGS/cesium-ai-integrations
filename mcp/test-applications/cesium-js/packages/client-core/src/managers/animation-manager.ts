...
import { wrapOperation, getParam } from "../shared/command-utils.js";
...
class CesiumAnimationManager implements ManagerInterface {
  ...
  createAnimationFromRoute(cmd: MCPCommand): MCPCommandResult {
    return wrapOperation(() => {
      const animationId = getParam<string>(cmd, "animationId");
      // Use animationId as the entity ID (single ID for both)
      const positionSamples = getParam<PositionSample[]>(
        cmd,
        "positionSamples",
      );
      const startTime = getParam<string | JulianDate>(cmd, "startTime");
      const stopTime = getParam<string | JulianDate>(cmd, "stopTime");
      const modelUri = getParam<string>(cmd, "modelUri");
      const showPath = getParam<boolean>(cmd, "showPath");
      const speedMultiplier = getParam<number>(cmd, "speedMultiplier", 10);
      const autoPlay = getParam<boolean>(cmd, "autoPlay");
      const trackCamera = getParam<boolean>(cmd, "trackCamera");
  ...
  removeAnimation(cmd: MCPCommand): MCPCommandResult {
    return wrapOperation(() => {
      ...
      try {
        ...
        const entity = getEntityById(this.viewer, animationId);
  ...
  configureAnimationPath(cmd: MCPCommand): MCPCommandResult {
    return wrapOperation(() => {
      const animationId = getParam<string>(cmd, "animationId");
      const leadTime = getParam<number | undefined>(cmd, "leadTime");
      const trailTime = getParam<number | undefined>(cmd, "trailTime");
      const width = getParam<number | undefined>(cmd, "width");
      const color = getParam<string | ColorRGBA | undefined>(cmd, "color");
      ...
      const configured = configureEntityPath(entity, {
        leadTime,
        trailTime,
        width,
        color,
  ...
  trackAnimationEntity(cmd: MCPCommand): MCPCommandResult {
    return wrapOperation(() => {
      const animationId = getParam<string>(cmd, "animationId");
      const range = getParam<number | undefined>(cmd, "range");
      const pitch = getParam<number | undefined>(cmd, "pitch");
      const heading = getParam<number | undefined>(cmd, "heading");
  ...
  getCommandHandlers(): Map<string, CommandHandler> {
    ...
    handlers.set("clock_control", (cmd: MCPCommand) => {
      const action = getParam<string>(cmd, "action");
    ...
    handlers.set("animation_camera_tracking", (cmd: MCPCommand) => {
      const track = getParam<boolean>(cmd, "track");
