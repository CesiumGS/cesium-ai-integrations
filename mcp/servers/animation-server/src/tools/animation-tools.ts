...
/**
 * Coordinate position used in route data
 */
...
/**
 * Route step with optional location and duration info
 */
interface RouteStep {
  instruction?: string;
  distance?: number;
  duration?: number;
  startLocation?: RoutePosition;
  endLocation?: RoutePosition;
}

/**
 * Route leg containing steps and location info
 */
interface RouteLeg {
  distance?: number;
  duration?: number;
...
/**
 * Unified route data structure used internally after normalization.
 * Handles both direct route objects and geolocation_route response wrappers.
 */
interface RouteData {
  summary?: string;
  distance?: number;
  duration?: number;
  polyline?: string | RoutePosition[] | Array<[number, number, number?]>;
  ...
  primaryRoute?: { travelMode?: string };
}
...
export function registerAnimationTools(
  ...
  server.registerTool(
    "animation_create_from_route",
    {
      ...
      description:
        "Create an animated entity following a route. First call geolocation_route tool to get route data, then pass the complete route object to this tool. Auto-selects 3D model (walking→Cesium Man, driving→car, cycling→bike). Supports simple start/end locations or detailed route with legs/steps.",
    ...
    async (args: Record<string, unknown>) => {
      ...
      try {
        ...
        const trackCamera = args.trackCamera as boolean | undefined;

        // Validate route data
        if (!validatedArgs.route) {
          throw new Error(
            "Route data is required. Please call geolocation_route first to get route data, then pass it to this tool.",
          );
        }

        // Handle both direct route object and routes array from geolocation_route response
        let routeData = validatedArgs.route as RouteData;
        if (
          routeData.routes &&
          ...
          routeData = {
            ...routeData.routes[0],
            travelMode:
              routeData.primaryRoute?.travelMode ||
              routeData.routes[0].travelMode ||
              "driving",
          };
        }

        // Auto-select model preset based on travel mode
        let modelPreset: "cesium_man" | "car" | "bike" | "airplane" =
          "cesium_man";
        if (validatedArgs.modelPreset && validatedArgs.modelPreset !== "auto") {
          modelPreset = validatedArgs.modelPreset as
            | "cesium_man"
            | "car"
            | "bike"
            | "airplane";
        ...
        if (route.polyline) {
          if (Array.isArray(route.polyline)) {
            ...
            const decimatedPolyline = decimateArray<
              RoutePosition | [number, number, number?]
            >(route.polyline, 500);
            console.error(
              `Processing ${originalLength} decoded polyline coordinates (decimated to ${decimatedPolyline.length})`,
            );

            const totalDuration =
              (route.duration || decimatedPolyline.length * 10) * 1000;
            const durationPerSegment =
              totalDuration / (decimatedPolyline.length - 1);
            ...
            for (let i = 0; i < decimatedPolyline.length; i++) {
              ...
              if (i < decimatedPolyline.length - 1) {
                currentTime +=
                  durationPerSegment / (validatedArgs.speedMultiplier || 10.0);
              }
            }
            ...
            console.error(
              `Decoded encoded polyline to ${decodedPositions.length} coordinates`,
            );

            const decimatedPositions = decimateArray(decodedPositions, 500);
            if (decimatedPositions.length < decodedPositions.length) {
              console.error(
                `Decimated ${decodedPositions.length} decoded polyline points to ${decimatedPositions.length}`,
              );
            }

            const totalDuration =
              (route.duration || decimatedPositions.length * 10) * 1000;
            const durationPerSegment =
              totalDuration / (decimatedPositions.length - 1);
          ...
          const decimatedPositions = decimateArray(route.positions, 500);
          console.error(
            `Processing ${originalLength} direct positions (decimated to ${decimatedPositions.length})`,
          );
          ...
          for (const leg of route.legs) {
            ...
            if (
              leg.steps &&
              leg.steps.length > 0 &&
              leg.steps[0].startLocation &&
              leg.steps[0].endLocation
            ) {
              ...
              for (const step of leg.steps) {
                const durationValue =
                  typeof step.duration === "number"
                    ? step.duration * 1000
                    : parseDuration(step.duration || "1 min");
                currentTime +=
                  durationValue / (validatedArgs.speedMultiplier || 10.0);
              ...
              const legDistance =
                leg.distance ||
                leg.steps.reduce(
                  (sum: number, s: RouteStep) => sum + (s.distance || 0),
                  0,
                );
              ...
              for (const step of leg.steps) {
                ...
                const progress =
                  legDistance > 0
                    ...
                    : accumulatedDistance / leg.steps.length;

                // Linear interpolation between leg start and end
                const longitude =
                  leg.startLocation.longitude +
                  (leg.endLocation.longitude - leg.startLocation.longitude) *
                    progress;
                const latitude =
                  leg.startLocation.latitude +
                  (leg.endLocation.latitude - leg.startLocation.latitude) *
                    progress;

                const stepDuration = (step.duration || 60) * 1000;
                currentTime +=
                  stepDuration / (validatedArgs.speedMultiplier || 10.0);
              ...
              currentTime +=
                legDuration / (validatedArgs.speedMultiplier || 10.0);
              positionSamples.push({
                time: new Date(currentTime).toISOString(),
          ...
          if (route.waypoints && Array.isArray(route.waypoints)) {
            const segmentDuration =
              ((route.duration || 60) * 1000) / (route.waypoints.length + 1);
            for (const waypoint of route.waypoints) {
              currentTime +=
                segmentDuration / (validatedArgs.speedMultiplier || 10.0);
              positionSamples.push({
                time: new Date(currentTime).toISOString(),
          ...
          // Add end location
          const finalDuration =
            ((route.duration || 60) * 1000) /
            (route.waypoints ? route.waypoints.length + 1 : 1);
          currentTime +=
            finalDuration / (validatedArgs.speedMultiplier || 10.0);
          positionSamples.push({
            time: new Date(currentTime).toISOString(),
          ...
          throw new Error(
            "Invalid route format: must provide either polyline (string/array), positions array, legs array, or startLocation/endLocation",
          );
        }

        if (positionSamples.length < 2) {
          throw new Error(
            `Insufficient position samples: got ${positionSamples.length}, need at least 2`,
          );
        }

        console.error(
          `Extracted ${positionSamples.length} position samples from route`,
        );
  ...
  server.registerTool(
    "animation_create_custom_path",
    {
      ...
      description:
        "Create a custom animated entity with manually specified position samples. Useful for creating complex animations, circular paths, or figure-eight patterns.",
    ...
    async (args: Record<string, unknown>) => {
      ...
      try {
        ...
        const animStartTime =
          validatedArgs.startTime || validatedArgs.positionSamples[0].time;
        const animStopTime =
          validatedArgs.stopTime ||
          validatedArgs.positionSamples[
            validatedArgs.positionSamples.length - 1
          ].time;
  ...
  server.registerTool(
    "animation_update_speed",
    {
      ...
      inputSchema: {
        ...
        multiplier: z
          .number()
          .describe("Speed multiplier (e.g., 2.0 for 2x speed)"),
      },
      outputSchema: AnimationStateResponseSchema.shape,
    },
      ...
      try {
        ...
        const output = {
          ...
          stats: {
            ...
            activeAnimations: animationsList.filter((a) => a.isAnimating)
              .length,
            responseTime,
          },
        };
        ...
        const animDetails = Array.from(animations.entries())
          .map(([id, anim]) => {
            const state = animationsList.find(
              (a) => a.entityId === anim.entityId,
            );
            return (
