...
export function registerAnimationCreate(
  ...
  server.registerTool(
    'animation_create',
    {
      ...
      description: 'Create an animated entity with custom position samples. Provide positionSamples array with timing and coordinates.',
    ...
    async (args) => {
      try {
        ...
        try {
          resolvedModelUri = resolveModelUri(modelPreset, validatedArgs.modelUri);
        } catch (error) {
          throw new Error(`Failed to resolve model URI: ${formatErrorMessage(error)}`);
        }
