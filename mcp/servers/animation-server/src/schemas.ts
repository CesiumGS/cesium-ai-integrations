...
export const PositionSampleSchema = z.object({
  ...
  longitude: z.number().describe('Longitude in degrees'),
  latitude: z.number().describe('Latitude in degrees'),
  height: z.number().optional().default(0).describe('Height in meters above WGS84 ellipsoid')
 ...
 */
export const PathGraphicsSchema = z.object({
  ...
  resolution: z.number().min(1).optional().default(60).describe('Sample resolution in seconds')
...
export const ModelConfigSchema = z.object({
  ...
  minimumPixelSize: z.number().min(0).optional().default(64).describe('Minimum pixel size for model'),
  heightOffset: z.number().optional().default(0).describe('Height offset above ground in meters')
...
export const AnimationConfigSchema = z.object({
  ...
  stopTime: z.string().optional().describe('Animation stop time (ISO 8601, defaults to last sample time)'),
  interpolationAlgorithm: z.enum(['LINEAR', 'LAGRANGE', 'HERMITE']).optional().default('LAGRANGE').describe('Position interpolation method'),
  autoOrient: z.boolean().optional().default(true).describe('Automatically face direction of travel'),
  showPath: z.boolean().optional().default(true).describe('Show path trail visualization'),
...
export const RouteAnimationConfigSchema = z.object({
  ...
  route: z.object({
    ...
    polyline: z.union([
      ...
      z.array(z.union([
        PositionSchema,
        z.tuple([z.number(), z.number(), z.number().optional()]).describe('[longitude, latitude, height]')
      ])).describe('Array of decoded coordinates')
    ]).optional().describe('Route geometry - encoded string or decoded coordinates'),
 ...
 */
export const CameraTrackingConfigSchema = z.object({
...
export const ClockControlSchema = z.object({
  ...
  clockRange: z.enum(['UNBOUNDED', 'CLAMPED', 'LOOP_STOP']).optional().describe('Clock boundary behavior')
}).describe('Clock control parameters');

/**
 * Path update configuration
 */
export const PathUpdateConfigSchema = z.object({
  animationId: z.string().describe('Animation ID'),
...
export const AnimationCreateResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
...
export const AnimationStateResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
...
export const AnimationListResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
...
export const CameraTrackingResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
...
export const CZMLExportResponseSchema = z.object({
  success: z.boolean(),
  message: z.string(),
