 ...
 *
 * Provider-specific fields are documented to indicate support levels.
...
export const PlaceSchema = z.object({
  id: z.string().describe("Place ID (format varies by provider)"),
  ...
  types: z
    .array(z.string())
    .optional()
    .describe("Place types (format and availability varies by provider)"),
  rating: z
    .number()
    .optional()
    .describe("Rating (0-5) - Provider-specific: Google only"),
  userRatingsTotal: z
    .number()
    .optional()
    .describe("Number of ratings - Provider-specific: Google only"),
  priceLevel: z
    .number()
    .optional()
    .describe("Price level (0-4) - Provider-specific: Google only"),
  openNow: z
    .boolean()
    .optional()
    .describe("Currently open - Provider-specific: Google only"),
  photos: z
    .array(z.string())
    .optional()
    .describe("Photo URLs or references - Provider-specific: Google only"),
  vicinity: z
    .string()
    .optional()
    .describe("Simplified address - Provider-specific"),
});
...
export const RouteSchema = z.object({
  ...
  polyline: z
    .string()
    .describe("Encoded polyline geometry (encoding format varies by provider)"),
  bounds: z
    .object({
  ...
  trafficInfo: z
    ...
    .optional()
    .describe("Real-time traffic information - Provider-specific: Google only"),
});
