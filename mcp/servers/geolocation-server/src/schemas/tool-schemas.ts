...
export const SearchInputSchema = z.object({
  query: z.string().min(1).describe('Search query (e.g., "pizza restaurants", "gyms near me")'),
  location: PositionSchema.optional().describe("Center point for location-biased search"),
  radius: z.number().min(0).max(50000).optional().describe("Search radius in meters (max 50km)"),
...
export const NearbySearchInputSchema = z.object({
  ...
  radius: z.number().min(0).max(50000).default(5000).describe("Search radius in meters (default 5km)"),
  types: z.array(PlaceTypeSchema).optional().describe("Filter by place types"),
  keyword: z.string().optional().describe('Keyword to filter results (e.g., "organic")'),
...
export const VisualizationOptionsSchema = z.object({
  ...
  routeColor: z.string().optional().describe("Route polyline color (CSS color or hex)"),
  routeWidth: z.number().min(1).max(20).default(5).describe("Route line width in pixels"),
  drawCorridor: z.boolean().default(false).describe("Draw route as polygon corridor"),
  flyToRoute: z.boolean().default(true).describe("Automatically fly camera to view route"),
});
