import type { RouteInput, Route, Position } from "../schemas/index.js";

/** Shape of a step object from the Google Routes API */
interface GoogleApiStep {
  navigationInstruction?: { instructions?: string };
  distanceMeters?: number;
  staticDuration?: string;
}

/** Shape of a leg object from the Google Routes API */
interface GoogleApiLeg {
  distanceMeters?: number;
  duration?: string;
  startLocation?: GoogleApiLocation;
  endLocation?: GoogleApiLocation;
  steps?: GoogleApiStep[];
}

/** Shape of a location object from the Google Routes API */
interface GoogleApiLocation {
  latLng?: { latitude?: number; longitude?: number };
}

/** Shape of a route object from the Google Routes API */
interface GoogleApiRoute {
  description?: string;
  distanceMeters?: number;
  duration?: string;
  ...
  legs?: GoogleApiLeg[];
  warnings?: string[];
  travelAdvisory?: { duration?: string };
}
...
export class RoutesService {
  ...
  constructor(apiKey?: string) {
    ...
    if (!this.apiKey) {
      console.error(
        "⚠️  Google Maps API key not set. Routing features will be limited.",
      );
    }
  }
  ...
  async computeRoute(input: RouteInput): Promise<Route[]> {
    ...
    const cached = this.getFromCache(cacheKey) as Route[] | null;
    if (cached) {
      return cached;
    }
   ...
   */
  private transformRoutes(apiRoutes: GoogleApiRoute[]): Route[] {
    return apiRoutes.map((route) => {
      ...
      const transformed: Route = {
        ...
        bounds,
        legs: legs.map((leg: GoogleApiLeg) => ({
          ...
          steps: leg.steps
            ? leg.steps.map((step: GoogleApiStep) => ({
   ...
   */
  private extractLocation(location: GoogleApiLocation | undefined): Position {
