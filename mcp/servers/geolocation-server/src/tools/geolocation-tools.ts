...
export function registerGeolocationTools(
  ...
  if (!communicationServer) {
    throw new Error('Geolocation tools require a communication server for browser visualization');
  }
  ...
  server.registerTool(
    'geolocation_search',
    {
      ...
      description: 'Search for places by name or type (e.g., "pizza restaurants", "gyms near me"). Supports location-biased search.',
    ...
    async (params) => {
      ...
      try {
        const places = await placesService.searchPlaces(params as any);
  ...
  server.registerTool(
    'geolocation_nearby',
    {
      ...
      description: 'Find places within a radius of a specific location. Supports filtering by type, rating, and open status.',
    ...
    async (params) => {
      ...
      try {
        const places = await placesService.searchNearby(params as any);
        const responseTime = Date.now() - startTime;
        ...
        const radius = (params as any).radius || 5000;
        return {
          content: [
            {
              type: 'text',
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        return {
          content: [{ type: 'text', text: `❌ Nearby search failed: ${errorMessage}` }],
          structuredContent: {
            success: false,
            places: [],
            message: errorMessage,
  ...
  server.registerTool(
    ...
    {
      title: 'Compute Route',
      description: 'Calculate optimal route between two locations with support for multiple travel modes (driving, walking, cycling, transit) and traffic awareness.',
    ...
    async (params) => {
      ...
      try {
        const routes = await routesService.computeRoute(params as any);
        const responseTime = Date.now() - startTime;
          ...
          for (let i = 0; i < routes.length; i++) {
            ...
            await communicationServer.executeCommand({
              ...
              route: {
                ...
                travelMode: (params as any).travelMode || 'driving'
              },
              options: {
          ...
          content: [
            {
              ...
              text: `✅ Found ${routes.length} route(s) (${responseTime}ms)\n\n${routes.map((r, i) => {
                ...
                return `${i === 0 ? '🔵 Best Route' : `Route ${i + 1}`}: ${r.summary}\n` +
                ...
                `\`\`\`json\n${JSON.stringify(coordinates.map(c => [c.longitude, c.latitude, c.height || 0]), null, 2)}\n\`\`\``;
              }).join('\n\n')}`
            }
          ],
          structuredContent: {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        return {
          content: [{ type: 'text', text: `❌ Route calculation failed: ${errorMessage}` }],
          structuredContent: {
            success: false,
            routes: [],
            message: errorMessage,
  ...
  server.registerTool(
    'geolocation_get_user_location',
    {
      ...
      description: 'Request the user\'s current location from the browser (requires user permission). Useful for "near me" queries.',
      inputSchema: {},
    ...
    async () => {
      ...
      try {
        ...
        }, 15000); // 15 second timeout for user interaction
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        return {
          content: [{ type: 'text', text: `❌ Location request failed: ${errorMessage}\n\n💡 Make sure to grant location permission in your browser` }],
          structuredContent: {
            success: false,
            message: errorMessage,
            timestamp: Date.now()
