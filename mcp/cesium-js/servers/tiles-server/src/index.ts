...
import { registerTilesTools } from "./tools/index.js";

const PORT = parseInt(
  process.env.PORT || process.env.TILES_SERVER_PORT || "3006",
);
...
async function main() {
  try {
    ...
    console.error(
      `Tiles Server starting with ${PROTOCOL.toUpperCase()} on port ${PORT} (strictPort: ${STRICT_PORT})`,
    );

    server.registerTools(registerTilesTools);

    await server.start();
  } catch (error) {
    console.error("Failed to start tiles server:", error);
    process.exit(1);
  }
}
