...
Coordinate Camera, Entity, Animation, and Geolocation MCP servers to create narrated flyover tours with multiple stops.
...
- [ ] Entity types: point, billboard, label, model, polygon, polyline, box, corridor, cylinder, ellipse, rectangle, wall
- [ ] Orbit at key stops? (Camera server)
- [ ] Specific time of day/lighting? (Animation server — `clock_control`, `globe_set_lighting`)
...
flowchart TB
    ...
    B -->|geolocation_search if needed| C[Optional: Set lighting]
    C -->|clock_control / globe_set_lighting| D[For each stop]
    D --> E[Add marker at location<br/>entity_add_label / entity_add_point / etc.]
    E --> F[Calculate viewpoint<br/>offset from marker]
    F --> G[Fly to VIEWPOINT<br/>camera_fly_to]
    G --> H[Orbit?<br/>camera_start_orbit]
    H --> I[Wait/delay]
    I --> J{More stops?}
    J -->|yes| D
    J -->|no| K[End tour]
...
| Tool | Use For |
|------|---------|
| `entity_add_point` | Simple colored dot markers |
| `entity_add_billboard` | Image markers pinned to the globe |
| `entity_add_label` | Text annotations at locations |
| `entity_add_model` | 3D glTF/glb models |
| `entity_add_polygon` | Filled areas (boundaries, zones) |
| `entity_add_polyline` | Lines connecting points (paths, routes) |
| `entity_add_box` | 3D box volumes |
| `entity_add_corridor` | Corridor shapes along paths |
| `entity_add_cylinder` | Cylindrical volumes |
| `entity_add_ellipse` | Circular/elliptical areas |
| `entity_add_rectangle` | Rectangular areas on the globe |
| `entity_add_wall` | Vertical wall surfaces |
...
```javascript
// 0. (Optional) Set time-of-day lighting via Animation server
clock_control({
  action: "configure",
  clock: {
    currentTime: "2026-06-21T10:00:00Z",
    multiplier: 1,
    shouldAnimate: false
  }
})
globe_set_lighting({ enableLighting: true })
...
| **Entity** | add_point, add_billboard, add_label, add_model, add_polygon, add_polyline, add_box, add_corridor, add_cylinder, add_ellipse, add_rectangle, add_wall, list, remove |
| **Animation** | clock_control (set time-of-day), globe_set_lighting (day/night effects) |
| **Geolocation** | search to resolve place names, route for path computation |
